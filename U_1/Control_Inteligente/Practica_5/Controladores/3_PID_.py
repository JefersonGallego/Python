...
def calculate_settling_time(time, response, reference, percent_overshoot=2):
...
def calculate_rise_time(time, response, reference):
    return time[np.argmax(response > 0.95 * reference)]

def pid_response_error(K, Ti, Td, dt=dt, time_end=time_end, reference=reference):
   # Función de transferencia de la planta (primer orden estándar)
    plant = cl.TransferFunction([1.853], [350.156, 1])
    #plant = cl.TransferFunction([0.0025], [1, 0.075, 0.00125])

    # Función de transferencia del controlador PID
    pid = cl.TransferFunction([K * Td, K, K / Ti], [1, 0])

    # Sistema en lazo cerrado
    system = cl.feedback(pid * plant)

    # Respuesta a una entrada en lazo cerrado (PID + Planta)
    time, response = cl.step_response(reference * system, T=np.arange(0, time_end, dt))

    # Respuesta a lazo Abierto (Planta)
    _, reference_response = cl.step_response(reference * plant, T=np.arange(0, time_end, dt))

    # Calcular las métricas de desempeño
    error = reference - response                                    # Error
    squared_error = error**2                                        # Error ^2
    absolute_error = np.abs(error)                                  # Error Absoluto     
    time_weighted_absolute_error = (time * absolute_error).sum()    # Error Absoluto Ponderado en el Tiempo
    mean_squared_error = squared_error.mean()                       # Error Medio Cuadrado
    ...
    print(f" Valor de Desempeño : {total_normalized_performance}")

    return total_normalized_performance

def pid_response_metrics(K, Ti, Td, dt=dt, time_end=time_end, reference=reference):
    # Función de transferencia de la planta (primer orden estándar)
    plant = cl.TransferFunction([1.853], [350.156, 1])
    #plant = cl.TransferFunction([0.0025], [1, 0.075, 0.00125])
    ...
    # Calcular las métricas de desempeño
    steady_state_error = abs(reference - response[-1])                     # Error estado estable
    overshoot = ((np.max(response) - reference) / reference) * 100         # Sobreimpulso
    settling_time = calculate_settling_time(time, response, reference)     # Tiempo de estabilización
    rise_time = calculate_rise_time(time, response, reference)             # Tiempo de Subida
    steady_state_error_in_percent = steady_state_error / reference * 100   # Error de estado estable

    # Ponderar las métricas de desempeño y penalizar según su magnitud
    performance_score = (
        (steady_state_error_in_percent / 100) * 0.3 +
        (overshoot / 100) * 0.3 +
    ...
    print(f" Valor de Desempeño: {normalized_performance}")

    return normalized_performance

# Aplicacion Algoritmo Genetico
poblacion = Poblacion(
                  n_individuos = 1,
                  n_variables  = 3,
                  limites_inf  = [1,0.1,0.01],
                  limites_sup  = [5,1,5],
                  verbose      = False
                  )

Control = poblacion.optimizar(
          funcion_objetivo   = pid_response_error,
          optimizacion       = "minimizar",
          n_generaciones     = 100,
...
vector_unos = np.ones(time_end)*reference

# Grafica
plt.grid()
plt.title('Respuesta: Lazo Abierto VS Control PID')
plt.plot(time, reference_response,'r-',linewidth=2)
plt.plot(time, vector_unos,'k-',linewidth=2)
plt.plot(time, response,'b--',linewidth=2)
plt.legend(['Lazo Abierto', 'Referencia',' Control PID'],loc = "lower right")
