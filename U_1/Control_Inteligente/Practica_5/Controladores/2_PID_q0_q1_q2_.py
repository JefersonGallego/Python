...
class Poblacion:
    ...
    def optimizar(self, funcion_objetivo, optimizacion, n_generaciones = 50,
        ...
        return(self.valor_variables_optimo)


def obtener_rendimiento_del_sistema(Q0, Q1, Q2, importancia_rise_time=0.7, importancia_settling_time=0.20, importancia_overshoot=0.10):
    def planta():
        num_planta = [1.8363452696627225]
        den_planta = [250.1560636804837, 1]
        planta_tf = cl.TransferFunction(num_planta, den_planta)
        return planta_tf

    def controlador_pid(Q0, Q1, Q2):
        k_p = Q1 + Q2
        k_i = Q1 * Q2
        k_d = Q0
        controlador_tf = cl.TransferFunction([k_d, k_p, k_i], [1, 0])
        return controlador_tf

    def respuesta_del_sistema(Q0, Q1, Q2):
        planta_tf = planta()
        controlador_tf = controlador_pid(Q0, Q1, Q2)
        sistema_ensamblado = cl.series(controlador_tf, planta_tf)
        sistema_realimentado = cl.feedback(sistema_ensamblado)
        tiempo, respuesta = cl.step_response(sistema_realimentado)
        return tiempo, respuesta

    def rendimiento_del_sistema(tiempo, respuesta):
        porcentaje_10 = 0.1 * respuesta[-1]
        porcentaje_90 = 0.9 * respuesta[-1]
        idx_10 = np.argmax(respuesta > porcentaje_10)
        idx_90 = np.argmax(respuesta > porcentaje_90)
        rise_time = tiempo[idx_90] - tiempo[idx_10]

        porcentaje_5 = 0.05 * respuesta[-1]
        idx_settling_start = np.argmax(respuesta > porcentaje_5)
        idx_settling_end = np.argmax(respuesta > respuesta[-1] * 0.05, axis=0)
        settling_time = tiempo[idx_settling_end] - tiempo[idx_settling_start]

        overshoot = (max(respuesta) - respuesta[-1]) / respuesta[-1] * 100

        # Normalizar los valores
        max_rise_time = 1.0  # Este valor debe ajustarse según tus preferencias y requisitos
        max_settling_time = 1.0  # Este valor debe ajustarse según tus preferencias y requisitos
        max_overshoot = 1.0  # Este valor debe ajustarse según tus preferencias y requisitos

        normalized_rise_time = rise_time / max_rise_time
        normalized_settling_time = settling_time / max_settling_time
        normalized_overshoot = overshoot / max_overshoot

        # Calcular el promedio ponderado
        total_importancia = importancia_rise_time + importancia_settling_time + importancia_overshoot
        rendimiento_promedio = (
            (normalized_rise_time * importancia_rise_time) +
            (normalized_settling_time * importancia_settling_time) +
            (normalized_overshoot * importancia_overshoot)
        ) / total_importancia
        
        print(f"Valor de Desempeño Normalizado: {rendimiento_promedio}")

        return rendimiento_promedio
                        
    # Obtener el rendimiento del sistema
    rendimiento = rendimiento_del_sistema(*respuesta_del_sistema(Q0, Q1, Q2))
...
poblacion = Poblacion(
                  n_individuos = 200,
                  n_variables  = 3,
                  limites_inf  = [0.01,0.01,0.01],
                  limites_sup  = [10,5.5,5.5],
                  verbose      = False
                  )

C = poblacion.optimizar(
          funcion_objetivo   = obtener_rendimiento_del_sistema,
          optimizacion       = "minimizar",
          n_generaciones     = 500,
          metodo_seleccion   = "tournament",
          elitismo           = 0.1,
...
Q1 = C[1]
Q2 = C[2]
# 0.011304767011685368 0.49936671953353484 0.5
def planta():
        #num_planta = [1]
        #den_planta = [1, 2.5, 5.8]
        num_planta = [1.8363452696627225]
        den_planta = [250.1560636804837, 1]
        planta_tf = cl.TransferFunction(num_planta, den_planta)
        return planta_tf
...
tiempo, salida = cl.step_response(sistema_realimentado)
tiempo_1, salida_1 = cl.step_response(planta_tf)


# Grafica
plt.grid()
plt.plot(tiempo, salida,'b-',linewidth=3)
#plt.plot(tiempo_1, salida_1,'r-',linewidth=3)
plt.legend(['Control PID ','Lazo Abierto '],loc = "lower right")
