...
def funcion_objetivo(K,den_1,den_2):
    s = sym.symbols('s', complex=True) # z=Simbolo
    ...
    sys1 = cl.tf(1, den_1)  # FT Ganacia Integral (1/Ti*S)
    sys2 = cl.tf(den_2,1)   # FT Ganacia Dervaritva (Td*S)
   ...
   # SettlingTime     = Tiempo de estabilización:     2. Tiempo de Asentamiento
   # SettlingMin      = Asentamiento mín
   # SettlingMax      = Asentamiento max
   # Overshoot        = Sobreimpulso                  3. Sobreimpulso 
   # Undershoot       = Subimpulso
   # Peak             = Pico
   # PeakTime         = Tiempo de pico
   # SteadyStateValue = Valor en estado estacionario  1. Valor en estado estable
    ...
    # Tiempo de Asentamiento Normalizado
    t,ye = cl.step_response(feed)
    t_1 = step.get('SettlingTime')
    ta = t_1/t[-1]
    ...
    # Calcular puntajes normalizados para cada parámetro
    puntaje_e = max(0, min(1, 1 - e))  # Asegurar que esté en el rango [0, 1]
    puntaje_sp = max(0, min(1, 1 - sp))  # Asegurar que esté en el rango [0, 1]
    puntaje_ta = max(0, min(1, ta / t[-1]))  # Asegurar que esté en el rango [0, 1]

    # Calcular el valor total ponderado
    valor_total = peso_e * puntaje_e + peso_sp * puntaje_sp + peso_ta * puntaje_ta
    print(f"Valor de Desempeño Normalizado: {valor_total}")
    return(valor_total)
...
y = S.get('SteadyStateValue')
e = 1 - y

# Tiempo de establecimiento
t_1 = S.get('SettlingTime')
ta = t_1/t[-1]
#print(t_1,t[-1],ta)

#Sobre impuslo
sp_1 = S.get('Overshoot')
sp = sp_1/100
#print(sp_1,sp)
...
#plt.plot(t,ye_1,'b-',linewidth=3)
plt.legend(['Control PID ',],loc = "lower right")
