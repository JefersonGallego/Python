...
true_valors = [1.5,1.6,1.8,1.1]

def fitness_function_classification(a,b,c,d, true_labels = true_valors ):
...
predicted_valors = poblacion.optimizar(
          funcion_objetivo   = fitness_function_classification,
...
#Para graficar
correct_predictions = sum(pred == true for pred, true in zip(predicted_valors, true_valors))
total_predictions = len(predicted_valors)
accuracy = (correct_predictions / total_predictions)*100

print(f"Valores Reales:      {true_valors}")
print(f"Valores Prediccion:  {predicted_valors}")
print(f"% Prediccion           {accuracy}")
#plt.show()
...
##     vec= vec[np.newaxis]
##     xtotal.append(vec)
##     yf=funcion_objetivo(vec[0,0],vec[0,1])
##     yt.append(float(np.array(yf)))
##
##
##ytotal.append(np.array(yt))
##fig = plt.figure(1) # Creamos la figura
...
##
##ax1 = p3.Axes3D(fig)
##ax1.plot_surface(x, y, ytotal, rstride=1,cstride=1)
