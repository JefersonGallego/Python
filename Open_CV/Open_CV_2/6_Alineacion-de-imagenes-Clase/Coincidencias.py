...
while True:
    ...
    # 1. Crear objeto comparador
    matcher = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck = True)
    matches = matcher.match(descriptor1, descriptor2) # Comparar Descripciones

    # 2. Ordenamos la lista
    matches = sorted(matches, key = lambda x: x.distance, reverse = False)

    # 3. Filtramos los resultados
    goodmatches = int(len(matches) * 0.1) # Unir 10 % de los descriptores
